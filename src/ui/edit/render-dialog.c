...
static gchar *bt_render_dialog_make_file_name(const BtRenderDialog *self, gint track) {
...
static void bt_render_dialog_record(const BtRenderDialog *self) {
  gchar *info;

  GST_INFO("recording to '%s'",self->priv->file_name);
  g_object_set(self->priv->sink_bin,"record-file-name",self->priv->file_name,NULL);
...
static void bt_render_dialog_record_init(const BtRenderDialog *self) {
  BtSetup *setup;
  BtMachine *machine;
  ...
  if((machine=bt_setup_get_machine_by_type(setup,BT_TYPE_SINK_MACHINE))) {
    g_object_get(machine,"machine",&self->priv->sink_bin,"adder-convert",&self->priv->convert,NULL);

    g_object_set(self->priv->sink_bin,
      "mode",BT_SINK_BIN_MODE_RECORD,
      // TODO(ensonic): this hangs :/
      //"mode",BT_SINK_BIN_MODE_PLAY_AND_RECORD,
      "record-format",self->priv->format,
      NULL);
     ...
     */
    g_object_set(self->priv->convert,"dithering",2,"noise-shaping",3,NULL);
  ...
  if(self->priv->mode==BT_RENDER_MODE_MIXDOWN) {
    ...
    self->priv->list=bt_setup_get_machines_by_type(setup,BT_TYPE_SOURCE_MACHINE);
    self->priv->track=0;
...
static void bt_render_dialog_record_done(const BtRenderDialog *self) {
  /* reset play mode */
  g_object_set(self->priv->sink_bin,
...
/* Run the rendering. */
static void bt_render_dialog_record_next(const BtRenderDialog *self) {
  ...
  self->priv->file_name=bt_render_dialog_make_file_name(self,self->priv->track);
  
  if(self->priv->mode==BT_RENDER_MODE_SINGLE_TRACKS) {
    BtSongInfo *song_info;
    gchar *track_name,*id;

    machine=BT_MACHINE(g_list_nth_data(self->priv->list,self->priv->track));
...
static void on_okay_clicked(GtkButton *button, gpointer user_data) {
  BtRenderDialog *self=BT_RENDER_DIALOG(user_data);
...
static void bt_render_dialog_init_ui(const BtRenderDialog *self) {
  ...
  gtk_container_add(GTK_CONTAINER(gtk_dialog_get_action_area(GTK_DIALOG(self))),self->priv->okay_button);
  ...
  g_signal_connect(self->priv->okay_button, "clicked", G_CALLBACK(on_okay_clicked), (gpointer)self);
  ...
  table=gtk_table_new(/*rows=*/7,/*columns=*/2,/*homogenous=*/FALSE);
  ...
  self->priv->dir_chooser=widget=gtk_file_chooser_button_new(_("Select a folder"), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  ...
  */
  ...
  gtk_table_attach(GTK_TABLE(table),GTK_WIDGET(self->priv->info),0,2,4,5,GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND, 2,1);

  self->priv->track_progress=GTK_PROGRESS_BAR(gtk_progress_bar_new());
  gtk_table_attach(GTK_TABLE(table),GTK_WIDGET(self->priv->track_progress),0,2,5,6,GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND, 2,1);

  // connect signal handlers    
  g_signal_connect(self->priv->song, "notify::play-pos", G_CALLBACK(on_song_play_pos_notify), (gpointer)self);
...
static void bt_render_dialog_dispose(GObject *object) {
  ...
  if(self->priv->song) {
    ...
    g_signal_handlers_disconnect_matched(self->priv->song,G_SIGNAL_MATCH_FUNC,0,0,NULL,on_song_play_pos_notify,NULL);    
    gst_object_unref(bin);
