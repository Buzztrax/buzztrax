...
struct _BtMainPageMachinesPrivate
{
  ...
  // DEBUG
  GtkWidget *hlower, *hupper, *hvalue, *hrel, *hpage;
  GtkWidget *vlower, *vupper, *vvalue, *vrel, *vpage;
  // DEBUG
};
...
static void
on_vvalue_changed (GtkEditable * editable, gpointer user_data)
{
  BtMainPageMachines *self = BT_MAIN_PAGE_MACHINES (user_data);
  gdouble v = g_ascii_strtod (gtk_entry_get_text (GTK_ENTRY (editable)), NULL);

  gtk_adjustment_set_value (self->priv->vadjustment, v);
}

static void
on_hvalue_changed (GtkEditable * editable, gpointer user_data)
{
  BtMainPageMachines *self = BT_MAIN_PAGE_MACHINES (user_data);
  gdouble v = g_ascii_strtod (gtk_entry_get_text (GTK_ENTRY (editable)), NULL);

  gtk_adjustment_set_value (self->priv->hadjustment, v);
}
...
update_scrolled_window (const BtMainPageMachines * self)
{
  BtMainPageMachinesPrivate *p = self->priv;
  // view size
  gdouble zvw = p->view_w / p->zoom;
  gdouble zvh = p->view_h / p->zoom;
  // the view (visible from vx .. vx+vw, vy .. vy+vh)
  gdouble vmix = gtk_adjustment_get_lower (p->hadjustment);
  gdouble vmax = gtk_adjustment_get_upper (p->hadjustment);
  gdouble vx = gtk_adjustment_get_value (p->hadjustment);
  gdouble vw = gtk_adjustment_get_page_size (p->hadjustment);
  gdouble vmiy = gtk_adjustment_get_lower (p->vadjustment);
  gdouble vmay = gtk_adjustment_get_upper (p->vadjustment);
  gdouble vy = gtk_adjustment_get_value (p->vadjustment);
  gdouble vh = gtk_adjustment_get_page_size (p->vadjustment);
  gdouble dx, dy, mx, my;
  // the bounding box
  gdouble bbmix, bbmax, bbmiy, bbmay;

  GST_WARNING ("vadj  : %3d .. %3d .. %3d, size: %3d", (gint) vmix, (gint) vx,
      (gint) vmax, (gint) vw);
  GST_WARNING ("hadj  : %3d .. %3d .. %3d, size: %3d", (gint) vmiy, (gint) vy,
      (gint) vmay, (gint) vh);
  GST_WARNING ("bb x: %3d .. %3d, y: %3d .. %3d", (gint) p->mi_x,
      (gint) p->ma_x, (gint) p->mi_y, (gint) p->ma_y);

  // relative scrollpos
  dx = vmax - vmix;
  if (dx >= vw) {
    mx = (vx - vmix) / (dx - vw);
    // allow to scroll bb out of view
    bbmix = p->mi_x - p->ma_x;
    bbmax = p->ma_x + p->ma_x;
  } else {
    mx = 0.5;
  ...
  // set new pos based on new bounds and old relative pos
  // FIXME: does not work the way it should mx,my should never get >1.0
  vx = bbmix + ((bbmax - bbmix) - zvw) * mx;
  vy = bbmiy + ((bbmay - bbmiy) - zvh) * my;
  //vx = CLAMP (vx, bbmix, (bbmax - zvw));
  //vy = CLAMP (vy, bbmiy, (bbmay - zvh));
  // if the window is small the middle is in the top, left
  // then it moves to the bottom, right
  // if the window is large it sticks in the bottom, right

  GST_WARNING ("view  : %3d,%3d .. %3d,%3d", (gint) vx, (gint) vy, (gint) vw,
      (gint) vh);
  GST_WARNING ("vadj  : %3d .. (%4.2f) %3d .. %3d, size: %3d", (gint) bbmix, mx,
      (gint) vx, (gint) bbmax, (gint) zvw);
  GST_WARNING ("hadj  : %3d .. (%4.2f) %3d .. %3d, size: %3d", (gint) bbmiy, my,
      (gint) vh, (gint) bbmay, (gint) zvh);

  // DEBUG
  gtk_entry_set_text (GTK_ENTRY (p->hlower),
      bt_persistence_strfmt_double (bbmix));
  gtk_entry_set_text (GTK_ENTRY (p->hvalue), bt_persistence_strfmt_double (vx));
  if (dx >= vw) {
    gtk_entry_set_text (GTK_ENTRY (p->hrel), bt_persistence_strfmt_double (mx));
  } else {
    gtk_entry_set_text (GTK_ENTRY (p->hrel), "---");
  }
  gtk_entry_set_text (GTK_ENTRY (p->hpage), bt_persistence_strfmt_double (zvw));
  gtk_entry_set_text (GTK_ENTRY (p->hupper),
      bt_persistence_strfmt_double (bbmax));

  gtk_entry_set_text (GTK_ENTRY (p->vlower),
      bt_persistence_strfmt_double (bbmiy));
  gtk_entry_set_text (GTK_ENTRY (p->vvalue), bt_persistence_strfmt_double (vy));
  if (dy >= vh) {
    gtk_entry_set_text (GTK_ENTRY (p->vrel), bt_persistence_strfmt_double (my));
  } else {
    gtk_entry_set_text (GTK_ENTRY (p->vrel), "---");
  }
  gtk_entry_set_text (GTK_ENTRY (p->vpage), bt_persistence_strfmt_double (zvh));
  gtk_entry_set_text (GTK_ENTRY (p->vupper),
      bt_persistence_strfmt_double (bbmay));
  // DEBUG

  // page size is how much of the scrollable part is visible
  gtk_adjustment_configure (p->hadjustment, vx, bbmix, bbmax, 1.0, 1.0, zvw);
  gtk_adjustment_configure (p->vadjustment, vy, bbmiy, bbmay, 1.0, 1.0, zvh);
}
...
static void
bt_main_page_machines_setup_grid (const BtMainPageMachines * self)
{
  gfloat cw = self->priv->view_w, ch = self->priv->view_h;

  // generate a grid one (pushing it to bottom)
  self->priv->grid = clutter_actor_new ();
  clutter_actor_set_size (self->priv->grid, cw, ch);
  clutter_actor_set_anchor_point_from_gravity (self->priv->grid,
      CLUTTER_GRAVITY_CENTER);
  clutter_actor_set_position (self->priv->grid, cw / 2.0, ch / 2.0);
  clutter_actor_add_child (self->priv->canvas, self->priv->grid);
  clutter_actor_set_child_below_sibling (self->priv->canvas, self->priv->grid,
      NULL);

  self->priv->grid_canvas = clutter_canvas_new ();
  clutter_canvas_set_size (CLUTTER_CANVAS (self->priv->grid_canvas), cw, ch);
    ...
    gfloat cw, gfloat ch, gdouble * x, gdouble * y)
{
  ...
  GST_WARNING ("bb: %4.2f,%4.2f .. %4.2f,%4.2f",
...
on_toolbar_zoom_fit_clicked (GtkButton * button, gpointer user_data)
{
  ...
  for (node = list; node; node = g_list_next (node)) {
    ...
    // get position
    g_object_get (machine, "properties", &properties, NULL);
    ma_x = self->priv->canvas_w / 2.0;
    ma_y = self->priv->canvas_h / 2.0;
    ...
    if (ma_y > ma_ye)
      ma_ye = ma_y;
    GST_DEBUG
        ("machines: x:%+6.4lf y:%+6.4lf -> ranging from x:%+6.4lf...%+6.4lf and y:%+6.4lf...%+6.4lf",
        ma_x, ma_y, ma_xs, ma_xe, ma_ys, ma_ye);
  }
  g_list_free (list);
  /* need to add machine extends + some space */
  GST_INFO ("machines ranging from x:%+6.4lf...%+6.4lf and y:%+6.4lf...%+6.4lf",
  ...
  GST_INFO ("x: (%+6.4lf-%+6.4lf)/2=%+6.4lf", pg_xl, (ma_xd * self->priv->zoom),
      ((pg_xl - (ma_xd * self->priv->zoom)) / 2.0));
  GST_INFO ("y: (%+6.4lf-%+6.4lf)/2=%+6.4lf", pg_yl, (ma_yd * self->priv->zoom),
      ((pg_yl - (ma_yd * self->priv->zoom)) / 2.0));
  c_x =
      (MACHINE_VIEW_SIZE_X + ma_xs) * self->priv->zoom - ((pg_xl -
          (ma_xd * self->priv->zoom)) / 2.0);
  c_y =
      (MACHINE_VIEW_SIZE_Y + ma_ys) * self->priv->zoom - ((pg_yl -
          (ma_yd * self->priv->zoom)) / 2.0);
  gtk_adjustment_set_value (self->priv->hadjustment, c_x);
  gtk_adjustment_set_value (self->priv->vadjustment, c_y);
  GST_INFO
      ("toolbar zoom_fit event occurred: zoom = %lf, center x/y = %+6.4lf,%+6.4lf",
      self->priv->zoom, c_x, c_y);
    ...
    gpointer user_data)
{
  ...
  gfloat delta[2] = { (vw - self->priv->view_w), (vh - self->priv->view_h) };

  // keep machines centered
  g_hash_table_foreach (self->priv->machines, machine_actor_move, delta);

  // size of the canvas (view)
  self->priv->view_w = vw;
  self->priv->view_h = vh;
  // initially page_size is not set to a good value
  gtk_adjustment_set_page_size (self->priv->hadjustment, vw / self->priv->zoom);
  gtk_adjustment_set_page_size (self->priv->vadjustment, vh / self->priv->zoom);

  // need to make stage+grid large enough to show grid when scrolling
  cw = 2.0 * (vw / self->priv->zoom);
  ch = 2.0 * (vh / self->priv->zoom);
  self->priv->canvas_w = cw;
  self->priv->canvas_h = ch;
  clutter_actor_set_size (self->priv->stage, cw, ch);
  // size of grid 
  clutter_actor_set_size (self->priv->grid, cw, ch);
  clutter_actor_set_position (self->priv->grid, cw / 2.0, ch / 2.0);
  clutter_canvas_set_size (CLUTTER_CANVAS (self->priv->grid_canvas), cw, ch);
...
on_vadjustment_changed (GtkAdjustment * adjustment, gpointer user_data)
{
  ...
  GST_WARNING ("ypos: %lf", val);
  store_scroll_pos (self, "ypos", val);

  // DEBUG
  g_signal_handlers_block_by_func (self->priv->vvalue, on_vvalue_changed,
      user_data);
  gtk_entry_set_text (GTK_ENTRY (self->priv->vvalue),
      bt_persistence_strfmt_double (val));
  g_signal_handlers_unblock_by_func (self->priv->vvalue, on_vvalue_changed,
      user_data);

  gdouble vmiy = gtk_adjustment_get_lower (adjustment);
  gdouble vmay = gtk_adjustment_get_upper (adjustment);
  gdouble vh = gtk_adjustment_get_page_size (adjustment);
  gdouble dy = vmay - vmiy;
  if (dy >= vh) {
    gtk_entry_set_text (GTK_ENTRY (self->priv->vrel),
        bt_persistence_strfmt_double ((val - vmiy) / (dy - vh)));
  } else {
    gtk_entry_set_text (GTK_ENTRY (self->priv->vrel), "---");
  }
  // DEBUG
...
on_hadjustment_changed (GtkAdjustment * adjustment, gpointer user_data)
{
  ...
  GST_WARNING ("xpos: %lf", val);
  store_scroll_pos (self, "xpos", val);

  // DEBUG
  g_signal_handlers_block_by_func (self->priv->hvalue, on_hvalue_changed,
      user_data);
  gtk_entry_set_text (GTK_ENTRY (self->priv->hvalue),
      bt_persistence_strfmt_double (val));
  g_signal_handlers_unblock_by_func (self->priv->hvalue, on_hvalue_changed,
      user_data);

  gdouble vmix = gtk_adjustment_get_lower (adjustment);
  gdouble vmax = gtk_adjustment_get_upper (adjustment);
  gdouble vw = gtk_adjustment_get_page_size (adjustment);
  gdouble dx = vmax - vmix;
  if (dx >= vw) {
    gtk_entry_set_text (GTK_ENTRY (self->priv->hrel),
        bt_persistence_strfmt_double ((val - vmix) / (dx - vw)));
  } else {
    gtk_entry_set_text (GTK_ENTRY (self->priv->hrel), "---");
  }
  // DEBUG
    ...
    const BtMainPages * pages)
{
  ...
  // add canvas
  table = gtk_table_new (2 + 1, 2 + 1, FALSE);
  // adjustments are not configured, need window-size for it
  self->priv->vadjustment = gtk_adjustment_new (0.0, 0.0, 10.0, 1.0, 1.0, 1.0);
  ...
  self->priv->hadjustment = gtk_adjustment_new (0.0, 0.0, 10.0, 1.0, 1.0, 1.0);
  ...
  gtk_table_attach (GTK_TABLE (table), GTK_WIDGET (box),
      2, 3, 0, 1, 0, GTK_EXPAND | GTK_FILL, 0, 0);

  self->priv->vlower = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->vvalue = entry = gtk_entry_new ();
  g_signal_connect (entry, "changed", G_CALLBACK (on_vvalue_changed),
      (gpointer) self);
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->vrel = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->vpage = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->vupper = entry = gtk_entry_new ();
  gtk_box_pack_end (box, entry, FALSE, FALSE, 6);

  box = GTK_BOX (gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0));
  gtk_table_attach (GTK_TABLE (table), GTK_WIDGET (box),
      0, 1, 2, 3, GTK_EXPAND | GTK_FILL, 0, 0, 0);

  self->priv->hlower = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->hvalue = entry = gtk_entry_new ();
  g_signal_connect (entry, "changed", G_CALLBACK (on_hvalue_changed),
      (gpointer) self);
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->hrel = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->hpage = entry = gtk_entry_new ();
  gtk_box_pack_start (box, entry, FALSE, FALSE, 6);
  self->priv->hupper = entry = gtk_entry_new ();
  gtk_box_pack_end (box, entry, FALSE, FALSE, 6);
  ...
  clutter_actor_set_size (self->priv->canvas, MACHINE_VIEW_SIZE_X,
      MACHINE_VIEW_SIZE_Y);
  //clutter_actor_set_position (self->priv->canvas, 0.0, 0.0);
  ...
  bt_main_page_machines_setup_grid (self);
    ...
    gint ypos)
{
  ...
  GST_WARNING ("showing popup at %d,%d", xpos, ypos);
