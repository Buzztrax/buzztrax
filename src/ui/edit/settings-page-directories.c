/* $Id: settings-page-directories.c,v 1.1 2007-08-27 20:17:49 ensonic Exp $
 *
 * Buzztard
...
static gboolean bt_settings_page_directories_init_ui(const BtSettingsPageDirectories *self) {
  BtSettings *settings;
...
BtSettingsPageDirectories *bt_settings_page_directories_new(const BtEditApplication *app) {
  ...
  if(!(self=BT_SETTINGS_PAGE_DIRECTORIES(g_object_new(BT_TYPE_SETTINGS_PAGE_DIRECTORIES,
    "app",app,
    "n-rows",4,
    "n-columns",3,
    "homogeneous",FALSE,
  ...
  // generate UI
  if(!bt_settings_page_directories_init_ui(self)) {
    goto Error;
  }
...
static void bt_settings_page_directories_get_property(GObject      *object,
                               guint         property_id,
                               GValue       *value,
                               GParamSpec   *pspec)
{
...
static void bt_settings_page_directories_set_property(GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
...
static void bt_settings_page_directories_class_init(BtSettingsPageDirectoriesClass *klass) {
  ...
  gobject_class->get_property = bt_settings_page_directories_get_property;
