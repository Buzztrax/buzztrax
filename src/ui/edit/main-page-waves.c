...
wavelevels_list_refresh (const BtMainPageWaves * self, const BtWave * wave)
{
  ...
  store =
      gtk_list_store_new (WAVELEVEL_TABLE_CT, G_TYPE_ULONG, G_TYPE_STRING,
      G_TYPE_ULONG, G_TYPE_ULONG, G_TYPE_LONG, G_TYPE_LONG);
  ...
  if (wave) {
    ...
    for (node = list; node; node = g_list_next (node), i++) {
      ...
      g_object_get (wavelevel,
          ...
          "loop-end", &loop_end, "rate", &rate, NULL);
      gtk_list_store_append (store, &tree_iter);
      gtk_list_store_set (store, &tree_iter,
          WAVELEVEL_TABLE_ID, i,
          WAVELEVEL_TABLE_ROOT_NOTE,
          gstbt_tone_conversion_note_number_2_string (root_note),
          WAVELEVEL_TABLE_LENGTH, length, WAVELEVEL_TABLE_RATE, rate,
          WAVELEVEL_TABLE_LOOP_START, loop_start, WAVELEVEL_TABLE_LOOP_END,
          loop_end, -1);
    }
    g_list_free (list);
  }
...
static void
on_playbin_state_changed (GstBus * bus, GstMessage * message,
    gpointer user_data)
{
  ...
  if ((store = gtk_tree_view_get_model (self->priv->waves_list))) {
    ...
    if (gtk_tree_model_get_iter_from_string (store, &iter, path_string)) {
      ...
      if ((wave = bt_wavetable_get_wave_by_index (self->priv->wavetable, id))) {
        ...
        gtk_list_store_set (GTK_LIST_STORE (store), &iter, WAVE_TABLE_NAME,
            new_text, -1);
        g_object_unref (wave);
  ...
  if ((wavelevel =
    ...
    if (root_note) {
      ...
      gtk_list_store_set (GTK_LIST_STORE (store), &iter,
          WAVELEVEL_TABLE_ROOT_NOTE,
          gstbt_tone_conversion_note_number_2_string (root_note), -1);
    ...
    gchar * path_string, gchar * new_text, gpointer user_data)
{
  ...
  if ((wavelevel =
    ...
    if (rate) {
      ...
      gtk_list_store_set (GTK_LIST_STORE (store), &iter, WAVELEVEL_TABLE_RATE,
          rate, -1);
      preview_update_seeks (self);
    }
    g_object_unref (wavelevel);
...
static void
on_browser_toolbar_stop_clicked (GtkToolButton * button, gpointer user_data)
{
  BtMainPageWaves *self = BT_MAIN_PAGE_WAVES (user_data);
      ...
      if (!self->priv->preview) {
        ...
        if (plugin_name) {
          self->priv->preview_sink =
              gst_element_factory_make (plugin_name, NULL);
          g_free (plugin_name);
        } else {
          self->priv->preview_sink =
    ...
    const BtMainPages * pages)
{
  ...
  gtk_tree_view_insert_column_with_attributes (self->priv->waves_list, -1,
      _("Ix"), renderer, "text", WAVE_TABLE_HEX_ID, NULL);
  renderer = gtk_cell_renderer_text_new ();
  gtk_cell_renderer_set_fixed_size (renderer, 1, -1);
  gtk_cell_renderer_text_set_fixed_height_from_font (GTK_CELL_RENDERER_TEXT
      (renderer), 1);
  ...
  g_signal_connect (renderer, "edited", G_CALLBACK (on_wave_name_edited),
      (gpointer) self);
  gtk_tree_view_insert_column_with_attributes (self->priv->waves_list, -1,
      _("Wave"), renderer, "text", WAVE_TABLE_NAME, NULL);
  ...
  self->priv->browser_stop = tool_item =
      GTK_WIDGET (gtk_tool_button_new_from_stock (GTK_STOCK_MEDIA_STOP));
  gtk_widget_set_name (tool_item, "Stop");
  gtk_tool_item_set_tooltip_text (GTK_TOOL_ITEM (tool_item),
      _("Stop playback of current sample"));
  gtk_toolbar_insert (GTK_TOOLBAR (self->priv->browser_toolbar),
      GTK_TOOL_ITEM (tool_item), -1);
  g_signal_connect (tool_item, "clicked",
      G_CALLBACK (on_browser_toolbar_stop_clicked), (gpointer) self);
  gtk_widget_set_sensitive (tool_item, FALSE);
  ...
  gtk_tree_view_insert_column_with_attributes (self->priv->wavelevels_list, -1,
      _("Root"), renderer, "text", WAVELEVEL_TABLE_ROOT_NOTE, NULL);
  ...
  gtk_cell_renderer_text_set_fixed_height_from_font (GTK_CELL_RENDERER_TEXT
      (renderer), 1);
  gtk_tree_view_insert_column_with_attributes (self->priv->wavelevels_list, -1,
      _("Length"), renderer, "text", WAVELEVEL_TABLE_LENGTH, NULL);
  renderer = gtk_cell_renderer_text_new ();
  gtk_cell_renderer_set_fixed_size (renderer, 1, -1);
  gtk_cell_renderer_text_set_fixed_height_from_font (GTK_CELL_RENDERER_TEXT
      (renderer), 1);
  g_object_set (renderer, "mode", GTK_CELL_RENDERER_MODE_EDITABLE, "editable",
      TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (self->priv->wavelevels_list, -1,
      _("Rate"), renderer, "text", WAVELEVEL_TABLE_RATE, NULL);
  ...
  g_object_set (renderer, "mode", GTK_CELL_RENDERER_MODE_EDITABLE, "editable",
      TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (self->priv->wavelevels_list, -1,
      _("Loop start"), renderer, "text", WAVELEVEL_TABLE_LOOP_START, NULL);
  g_signal_connect (renderer, "edited",
  ...
  g_object_set (renderer, "mode", GTK_CELL_RENDERER_MODE_EDITABLE, "editable",
      TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (self->priv->wavelevels_list, -1,
      _("Loop end"), renderer, "text", WAVELEVEL_TABLE_LOOP_END, NULL);
  g_signal_connect (renderer, "edited",
...
bt_main_page_waves_new (const BtMainPages * pages)
{
  ...
  gst_bus_add_signal_watch_full (bus, G_PRIORITY_HIGH);
  g_signal_connect (bus, "message::state-changed",
      G_CALLBACK (on_playbin_state_changed), (gpointer) self);
  gst_object_unref (bus);
