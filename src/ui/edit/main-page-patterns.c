    ...
    gpointer user_data)
{
  ...
  if (bt_change_log_is_active (self->priv->change_log)) {
    ...
    if (self->priv->global_params) {
      ...
      pattern_columns_log_undo_redo (self, fmt, self->priv->global_params, &str,
          NULL);
    }
    for (v = 0; v < voices; v++) {
      g_snprintf (fmt, MAX_CHANGE_LOGGER_METHOD_LEN,
          "set_voice_events \"%s\",\"%s\",0,%u,%u", mid, pid, end, v);
      pattern_columns_log_undo_redo (self, fmt, self->priv->voice_params, &str,
          NULL);
    }
  ...
  GST_WARNING ("removed pattern: %p,pattern->ref_ct=%d", pattern,
    ...
    gpointer user_data)
{
  ...
  if (event->keyval == GDK_Return) {    /* GDK_KP_Enter */
    ...
    bt_child_proxy_set (main_window, "pages::page", BT_MAIN_PAGES_SEQUENCE_PAGE,
        NULL);
...
static gboolean
on_pattern_table_button_press_event (GtkWidget * widget, GdkEventButton * event,
    gpointer user_data)
{
  BtMainPagePatterns *self = BT_MAIN_PAGE_PATTERNS (user_data);
  gboolean res = FALSE;
    ...
    guint row, guint track, guint param, guint digit, gfloat value)
{
  ...
  if (column_data)
    str =
        ((BtPatternEditorColumnConverters *) column_data)->float_to_str (value,
        column_data);
  else if (value != group->columns[param].def)
    ...
    const BtMainPages * pages)
{
  ...
  g_signal_connect (settings, "notify::toolbar-style",
      G_CALLBACK (on_toolbar_style_changed), (gpointer) toolbar);
