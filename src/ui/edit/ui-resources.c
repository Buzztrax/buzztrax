...
static void bt_ui_resources_dispose(GObject *object) {
  ...
  GST_DEBUG("  pb->ref_counts: %d, %d, %d", 
    self->priv->source_machine_pixbuf?G_OBJECT_REF_COUNT(self->priv->source_machine_pixbuf):0,
    self->priv->processor_machine_pixbuf?G_OBJECT_REF_COUNT(self->priv->processor_machine_pixbuf):0,
    self->priv->sink_machine_pixbuf?G_OBJECT_REF_COUNT(self->priv->sink_machine_pixbuf):0);
