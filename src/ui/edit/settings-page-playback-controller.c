...
static void on_controller_list_cursor_changed(GtkTreeView *treeview,gpointer user_data) {
...
static void on_activate_master_toggled(GtkCellRendererToggle *cell_renderer,gchar *path, gpointer user_data) {
  BtSettingsPagePlaybackController *self=BT_SETTINGS_PAGE_PLAYBACK_CONTROLLER(user_data);
  GtkTreeIter iter;
  GtkTreeModel *store;
...
static void on_activate_slave_toggled(GtkCellRendererToggle *cell_renderer,gchar *path, gpointer user_data) {
  BtSettingsPagePlaybackController *self=BT_SETTINGS_PAGE_PLAYBACK_CONTROLLER(user_data);
  GtkTreeIter iter;
  GtkTreeModel *store;
  gboolean active; // gtk_cell_renderer_toggle_get_active(cell_renderer); <- this does not seem to work
  ...
  switch(self->priv->page) {
    case CONTROLLER_PAGE_UPNP:
      g_object_get(self->priv->settings,"coherence-upnp-active",&active_setting,NULL);
      GST_INFO("upnp active changes from %d -> %d",active_setting,active);
      if(active!=active_setting) {
        g_object_set(self->priv->settings,"coherence-upnp-active",active,NULL);
      }
...
static void on_port_changed(GtkSpinButton *spinbutton,gpointer user_data) {
  ...
  g_object_set(self->priv->settings,"coherence-upnp-port",gtk_spin_button_get_value_as_int(spinbutton),NULL);
}
...
static void bt_settings_page_playback_controller_init_ui(const BtSettingsPagePlaybackController *self) {
  ...
  g_signal_connect(self->priv->controller_list,"cursor-changed",G_CALLBACK(on_controller_list_cursor_changed),(gpointer)self);
  ...
  gtk_tree_view_insert_column_with_attributes(self->priv->controller_list,-1,_("Master"),renderer,
    "sensitive",CONTROLLER_LIST_CAN_MASTER,
    "active",CONTROLLER_LIST_MASTER,
    NULL);
  g_signal_connect(renderer, "toggled", G_CALLBACK(on_activate_master_toggled), (gpointer)self);
  renderer=gtk_cell_renderer_toggle_new();
  gtk_tree_view_insert_column_with_attributes(self->priv->controller_list,-1,_("Slave"),renderer,
    "sensitive",CONTROLLER_LIST_CAN_SLAVE,
    "active",CONTROLLER_LIST_SLAVE,
    NULL);
  g_signal_connect(renderer, "toggled", G_CALLBACK(on_activate_slave_toggled), (gpointer)self);
  
  selection=gtk_tree_view_get_selection(self->priv->controller_list);
  gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
  gtk_container_add(GTK_CONTAINER(scrolled_window),GTK_WIDGET(self->priv->controller_list));
  gtk_table_attach(GTK_TABLE(self),GTK_WIDGET(scrolled_window), 1, 3, 1, 2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND, 2,1);
  
  // add data model
  store=gtk_list_store_new(6,G_TYPE_UINT,G_TYPE_STRING,G_TYPE_BOOLEAN,G_TYPE_BOOLEAN,G_TYPE_BOOLEAN,G_TYPE_BOOLEAN);
  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter,
                    CONTROLLER_LIST_ID,    CONTROLLER_PAGE_UPNP,
  ...
  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter,
                    CONTROLLER_LIST_ID,    CONTROLLER_PAGE_JACK,
                    // don't translate, this is a product
  ...
  gtk_table_attach(GTK_TABLE(self),GTK_WIDGET(self->priv->controller_pages), 1, 3, 2, 3, GTK_FILL,GTK_FILL, 2,1);
  ...
  gtk_table_attach(GTK_TABLE(table),label, 0, 2, 1, 2, GTK_FILL,GTK_SHRINK, 2,1);


  // jack transport tab
  plugin_name=bt_settings_determine_audiosink_name(self->priv->settings);
  str=g_strdup_printf("Jack transport requires that the jackaudiosink is active."
