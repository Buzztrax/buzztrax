    ...
    gpointer user_data)
{
  ...
  if ((column == self->priv->cursor_column) && (row == self->priv->cursor_row)) {
    bg_col = self->priv->cursor_bg;
  } else if ((column >= self->priv->selection_start_column)
      && (column <= self->priv->selection_end_column)
      && (row >= self->priv->selection_start_row)
      && (row <= self->priv->selection_end_row)) {
...
{
  /* delay the action */
  g_idle_add_full (G_PRIORITY_HIGH_IDLE, on_sequence_table_cursor_changed_idle,
      user_data, NULL);
}
  ...
  if (sequence_view_get_current_pos (self, &row, &track)) {
    ...
    if (event->keyval == GDK_space || event->keyval == GDK_period) {
      ...
      if (track > 0) {
        ...
        if ((row < length)
            && (pattern =
                bt_sequence_get_pattern (self->priv->sequence, row,
                    track - 1))) {
          GST_INFO ("show pattern");
          if (BT_IS_PATTERN (pattern)) {
            bt_main_page_patterns_show_pattern (patterns_page,
                (BtPattern *) pattern);
          }
          g_object_unref (pattern);
        } else {
          GST_INFO ("show machine");
          bt_main_page_patterns_show_machine (patterns_page,
              self->priv->machine);
        }
        g_object_unref (patterns_page);
    ...
    GdkEventButton * event, gpointer user_data)
{
  ...
  if (event->button == 1) {
    if (gtk_tree_view_get_bin_window (GTK_TREE_VIEW (widget)) ==
      ...
      GtkTreeViewColumn *column;
      gulong modifier =
          (gulong) event->state & (GDK_CONTROL_MASK | GDK_MOD4_MASK);
