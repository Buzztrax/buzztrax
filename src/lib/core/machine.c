                        ...
                        const BtMachineState new_state)
{
  ...
  if ((new_state == BT_MACHINE_STATE_BYPASS) && (!BT_IS_PROCESSOR_MACHINE(self)))
    return FALSE;
                         ...
                         GstPad *sink)
{
  ...
  if (GST_PAD_LINK_FAILED(plr = gst_pad_link(src, sink)))
  {
    ...
    GST_WARNING_OBJECT(self, "%s", bt_gst_debug_pad_link_return(plr, src, sink));
    return FALSE;
  }
  return TRUE;
}
                          ...
                          const BtMachinePart pos)
{
  ...
  else if ((pre == -1) && (post != -1))
  {
    ...
    if (!res)
    {
      ...
      {
        if ((wire =
                 (self->dst_wires ? (BtWire *)(self->dst_wires->data) : NULL)))
        {
  ...
  else if ((post == -1) && (pre != -1))
  {
    ...
    if (!res)
    {
      ...
      {
        if ((wire =
                 (self->src_wires ? (BtWire *)(self->src_wires->data) : NULL)))
        {
          if (!(res = bt_wire_reconnect(wire)))
          {
            GST_WARNING_OBJECT(self,
...
bt_machine_resize_voices(const BtMachine *const self, const gulong old_voices)
{
  ...
  GST_INFO_OBJECT(self, "changing machine %" G_OBJECT_REF_COUNT_FMT ", voices from %ld to %ld",
  ...
  if (old_voices < new_voices)
  {
    ...
    for (v = old_voices; v < new_voices; v++)
    {
                                                  ...
                                                  v)))
      {
        ...
        if ((properties =
                 g_object_class_list_properties(G_OBJECT_CLASS(GST_OBJECT_GET_CLASS(voice_child)),
...
bt_machine_init_prefs_params(const BtMachine *const self)
{
  ...
  if ((properties =
           g_object_class_list_properties(G_OBJECT_CLASS(GST_ELEMENT_GET_CLASS(machine)), &number_of_properties)))
  {
    guint i, j;
    gboolean skip;
...
bt_machine_init_global_params(const BtMachine *const self)
{
           ...
           g_object_class_list_properties(G_OBJECT_CLASS(GST_ELEMENT_GET_CLASS(machine)), &number_of_properties)))
  {
    ...
    if (GSTBT_IS_CHILD_BIN(self->priv->machines[PART_MACHINE]))
    {
      ...
      if ((voice_child =
               gst_child_proxy_get_child_by_index(GST_CHILD_PROXY(self->priv->machines[PART_MACHINE]), 0)))
      {
        child_properties =
            g_object_class_list_properties(G_OBJECT_CLASS(GST_OBJECT_GET_CLASS(voice_child)), &number_of_child_properties);
    ...
    for (i = 0; i < number_of_properties; i++)
    {
      ...
      else if (child_properties)
      {
        ...
        for (j = 0; j < number_of_child_properties; j++)
        {
          ...
          {
            GST_DEBUG("    skipping global_param [%d] \"%s\", due to voice_param[%d]",
...
BtMachine *bt_machine_clone(BtMachine *const self)
{
  ...
  machine = BT_MACHINE(bt_persistence_load(G_OBJECT_TYPE(self), NULL, node,
 ...
 * bt_pattern_new().
 */
void bt_machine_add_pattern(const BtMachine *const self,
 ...
 */
void bt_machine_remove_pattern(const BtMachine *const self,
                               const BtCmdPattern *const pattern)
{
  GList *node;
  g_return_if_fail(BT_IS_MACHINE(self));
...
bt_machine_get_unique_pattern_name(const BtMachine *const self)
{
  ...
  } while ((pattern = bt_machine_get_pattern_by_name(self, name)) && (i < 100));
  g_object_try_unref(pattern);
 ...
 */
void bt_machine_update_default_param_value(BtMachine *self,
 ...
 *
 */
void bt_machine_varargs_to_constructor_params(va_list var_args, gchar *id,
      ...
      if (!xmlNodeIsText(node))
      {
                          ...
                          24))
        {
          ...
          for (child_node = node->children; child_node;
               child_node = child_node->next)
          {
            if ((!xmlNodeIsText(child_node)) && (!strncmp((char *)child_node->name,
                        ...
                        GValue *const value, GParamSpec *const pspec)
{
  ...
  case MACHINE_PRETTY_NAME:
  {
    GstPluginFeature *feature =
        (GstPluginFeature *)gst_element_get_factory(self->priv->machines[PART_MACHINE]);
