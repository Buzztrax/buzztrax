...
//-- gobject ref-count debugging
#if 0
static GQuark ref_tags_quark=0;

static void GST_GNUC_CONSTRUCTOR object_ref_tag_init(void) {
  ref_tags_quark=g_quark_from_static_string("Bt::gobject-ref-tags");
}

gpointer bt_gobject_tag_ref(gpointer object,const gchar *tag) {
  GHashTable *ref_tags=g_object_get_qdata((GObject *)object,ref_tags_quark);
  guint count;
  
  if(G_UNLIKELY(!ref_tags)) {
    ref_tags=g_hash_table_new(g_str_hash,NULL);
    g_object_set_qdata_full((GObject *)object,ref_tags_quark,ref_tags,(GDestroyNotify)g_hash_table_destroy);
    count=1;
  }
  else {
    count=1+GPOINTER_TO_UINT(g_hash_table_lookup(ref_tags,tag));
  }
  g_hash_table_insert(ref_tags,(gpointer)tag,GUINT_TO_POINTER(count));
  
  return object;
}

void bt_gobject_tagged_unref(gpointer object,const gchar *tag) {
  GHashTable *ref_tags=g_object_get_qdata((GObject *)object,ref_tags_quark);

  if(ref_tags) {
    guint count=GPOINTER_TO_UINT(g_hash_table_lookup(ref_tags,tag));

    if(count) {
      g_hash_table_insert(ref_tags,(gpointer)tag,GUINT_TO_POINTER(count-1));
    }
    else {
      GST_WARNING("removing a unknown ref-tag %s on a %s",tag,G_OBJECT_TYPE_NAME(object));
    }
  }
...
static void print_ref_leaks(gpointer key,gpointer value, gpointer user_data) {
  GST_WARNING("%d ref-leaks with ref-tag %s on a %s",GPOINTER_TO_UINT(value),key,G_OBJECT_TYPE_NAME(user_data));
}

void bt_gobject_check_refs(gpointer object) {
  GHashTable *ref_tags=g_object_get_qdata((GObject *)object,ref_tags_quark);

  if(ref_tags && g_hash_table_size(ref_tags)) {
    // check pending refs
    g_hash_table_foreach(ref_tags,print_ref_leaks,object);
  }
  else {
    GST_WARNING("%d ref-leaks without ref-tags on a %s",G_OBJECT_REF_COUNT(object),G_OBJECT_TYPE_NAME(object));
  }
}
#endif
