 ...
 */
static gboolean bt_parameter_group_get_property_meta_value(GValue * const value, GParamSpec * const property, const GQuark key) {
  gboolean res=FALSE;
...
gboolean bt_parameter_group_is_param_trigger(const BtParameterGroup * const self, const gulong index) {
  ...
  if(!(self->priv->flags[index]&GSTBT_PROPERTY_META_STATE)) return(TRUE);
  return(FALSE);
}
...
gboolean bt_parameter_group_is_param_no_value(const BtParameterGroup * const self, const gulong index, GValue * const value) {
  ...
  if(!BT_IS_GVALUE(&self->priv->no_val[index])) return(FALSE);

  if(gst_value_compare(&self->priv->no_val[index],value)==GST_VALUE_EQUAL) return(TRUE);
  return(FALSE);
}
...
void bt_parameter_group_get_param_details(const BtParameterGroup * const self, const gulong index, GParamSpec **pspec, GValue **min_val, GValue **max_val) {
  ...
  if(min_val || max_val) {
    ...
    if(GSTBT_IS_PROPERTY_META(self->priv->parents[index])) {
      if(min_val) done=bt_parameter_group_get_property_meta_value(*min_val,property,gstbt_property_meta_quark_min_val);
      if(max_val) {
        if(!bt_parameter_group_get_property_meta_value(*max_val,property,gstbt_property_meta_quark_max_val)) {
...
glong bt_parameter_group_get_wave_param_index(const BtParameterGroup * const self) {
  ...
  for(i=0;i<params;i++) {
    if(flags[i]&GSTBT_PROPERTY_META_WAVE) return(i);
  }
  return(-1);
}
 ...
 */
static gboolean bt_parameter_group_has_param_default_set(const BtParameterGroup * const self, const gulong index) {
  return GPOINTER_TO_INT(g_object_get_qdata(self->priv->parents[index],self->priv->quarks[index]));
}
...
void bt_parameter_group_set_param_default(const BtParameterGroup * const self,const gulong index) {
  ...
  if(bt_parameter_group_has_param_default_set(self,index)) {
    ...
    bt_parameter_group_controller_change_value(self,index,G_GUINT64_CONSTANT(0),NULL);
  }
}
...
gchar *bt_parameter_group_describe_param_value(const BtParameterGroup * const self, const gulong index, GValue * const event) {
  ...
  if(GSTBT_IS_PROPERTY_META(self->priv->parents[index])) {
    return gstbt_property_meta_describe_property(GSTBT_PROPERTY_META(self->priv->parents[index]),index,event);
  }
  return NULL;
}
 ...
 */
void bt_parameter_group_controller_change_value(const BtParameterGroup * const self, const gulong ix, const GstClockTime timestamp, GValue *value) {
  ...
  if(G_UNLIKELY(!timestamp)) {
    if(!value) {
      ...
      // (bt_parameter_group_has_param_default_set)
      g_object_set_qdata(param_parent,self->priv->quarks[ix],GINT_TO_POINTER(TRUE));
      GST_INFO("set global default for param %lu:%s",ix,param_name);
    }
    else {
      // we set a real value for ts=0, no need to update the default
      g_object_set_qdata(param_parent,self->priv->quarks[ix],GINT_TO_POINTER(FALSE));
    }
  }
  ...
  if(value) {
    ...
    if(G_UNLIKELY(value==&def_value)) {
      ...
      if(!add) {
        if (!is_trigger) {
          ...
          GST_LOG("set controller: %"GST_TIME_FORMAT" param %s:%s",GST_TIME_ARGS(G_GUINT64_CONSTANT(0)),g_type_name(PARAM_TYPE(ix)),param_name);
          gst_interpolation_control_source_set(cs,G_GUINT64_CONSTANT(0),&def_value);
          g_value_unset(&def_value);
        }
        else {
          gst_interpolation_control_source_set(cs,G_GUINT64_CONSTANT(0),&self->priv->no_val[ix]);
        }
      }
    }
    else {
      ...
      GST_LOG("set controller: %"GST_TIME_FORMAT" param %s:%s",GST_TIME_ARGS(timestamp),g_type_name(PARAM_TYPE(ix)),param_name);
    ...
    GST_LOG("unset controller: %"GST_TIME_FORMAT" param %s:%s",GST_TIME_ARGS(timestamp),g_type_name(PARAM_TYPE(ix)),param_name);
    if(controller_rem_value(cs,timestamp,has_default)) {
      gst_controller_set_control_source(self->priv->controller[ix],param_name,NULL);
      g_object_unref(cs);
...
void bt_parameter_group_randomize_values(const BtParameterGroup * const self) {
  ...
  for(i=0;i<num_params;i++) {
    bt_g_object_randomize_parameter(self->priv->parents[i],self->priv->params[i]);
  }
}
...
static void bt_parameter_group_constructed(GObject *object) {
  ...
  self->priv->control_sources=(GstInterpolationControlSource **)g_new0(gpointer,num_params);
  ...
  for (i=0;i<num_params;i++) {
    ...
    qname=g_strdup_printf("%s::%s",G_OBJECT_TYPE_NAME(parent),param->name);
    ...
    if(GSTBT_IS_PROPERTY_META(parent)) {
      gconstpointer const has_meta=g_param_spec_get_qdata(param,gstbt_property_meta_quark);

      if(has_meta) {
        self->priv->flags[i]=GPOINTER_TO_INT(g_param_spec_get_qdata(param,gstbt_property_meta_quark_flags));
        if(!(bt_parameter_group_get_property_meta_value(&self->priv->no_val[i],param,gstbt_property_meta_quark_no_val))) {
          GST_WARNING("can't get no-val property-meta for param [%u/%lu] \"%s\"",i,num_params,param->name);
        }
      }
    }

    // use the properties default value for triggers as a no_value
    if(!BT_IS_GVALUE(&self->priv->no_val[i]) && !(param->flags&G_PARAM_READABLE)) {
...
static void bt_parameter_group_set_property(GObject * const object, const guint property_id, const GValue * const value, GParamSpec * const pspec) {
...
static void bt_parameter_group_get_property(GObject * const object, const guint property_id, GValue * const value, GParamSpec * const pspec) {
  const BtParameterGroup * const self = BT_PARAMETER_GROUP(object);
  return_if_disposed();
...
static void bt_parameter_group_dispose(GObject * const object) {
  const BtParameterGroup * const self = BT_PARAMETER_GROUP(object);
  gulong i;
...
static void bt_parameter_group_finalize(GObject * const object) {
  const BtParameterGroup * const self = BT_PARAMETER_GROUP(object);
  GValue *v;
  gulong i;
...
static void bt_parameter_group_init(BtParameterGroup *self) {
  GST_DEBUG("!!!! self=%p",self);
  self->priv = G_TYPE_INSTANCE_GET_PRIVATE(self, BT_TYPE_PARAMETER_GROUP, BtParameterGroupPrivate);
}

static void bt_parameter_group_class_init(BtParameterGroupClass * const klass) {
