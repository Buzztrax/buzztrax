/* $Id: input-device.c,v 1.1 2007-03-11 20:23:29 ensonic Exp $
 *
 * Buzztard
 ...
 */
BtIcInputDevice *btic_input_device_new(const gchar *udi,const gchar *name) {
  BtIcInputDevice *self=BTIC_INPUT_DEVICE(g_object_new(BTIC_TYPE_INPUT_DEVICE,"udi",udi,"name",name,NULL));
  if(!self) {
    goto Error;
  }
  return(self);
