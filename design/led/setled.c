 ...
 */
static gint led_set(gint and, gint or) {
  char current;
  char ref;
  char new;
  ...
  if (type==TYPE_GRAPHICS) {
    ...
    if (ioctl(fd,KDGKBLED,&ref)) {
      fprintf(stderr,"KDGKBLED on tty %s failed (%s)",TTY_NAME,g_strerror(errno));
      return -1;
    }
  }
...
gint main(gint argc, gchar **argv) {
  ...
  for(i=0;i<8;i++) {
    printf("SCR %d CAP %d NUM %d\n", ((i&LED_SCR)==LED_SCR), ((i&LED_CAP)==LED_CAP), ((i&LED_NUM)==LED_NUM));
    led_set(0,i);
    usleep(250000);
  }
  led_set(0,saved);
