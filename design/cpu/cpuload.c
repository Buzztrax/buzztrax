  ...
  for (l = 0; l < 100; l++) {
    ...
    while (y++ < 15) {
      ...
      for (x = 0; x++ < 84;) {
        ...
        for (i = k = r = 0;
            j = r * r - i * i - 2 + x / 25, i =
            2 * r * i + y / 10, j * j + i * i < 11 && k++ < 111; r = j);
      }
    }
    //usleep(50);
    ...
    // percentage
    cpuload =
        (int) gst_util_uint64_scale (tuser + tsys, G_GINT64_CONSTANT (100),
        treal);
    printf ("real %" GST_TIME_FORMAT ", user %" GST_TIME_FORMAT ", sys %"
        GST_TIME_FORMAT " => cpuload %d\n", GST_TIME_ARGS (treal),
