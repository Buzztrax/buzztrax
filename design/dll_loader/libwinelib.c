...
typedef NTSTATUS WINAPI (*NtProtectVirtualMemory_ptr) (HANDLE, PVOID *, ULONG *,
    ULONG, ULONG *);
typedef NTSTATUS WINAPI (*NtFreeVirtualMemory_ptr) (HANDLE, PVOID, ULONG *,
    ULONG);
typedef NTSTATUS WINAPI (*NtAllocateVirtualMemory_ptr) (HANDLE, PVOID *, PVOID,
    ULONG *, ULONG, ULONG);
...
static jmp_buf jump;
static void (*sharedwine_signal_handler) (int signal, siginfo_t *, void *) =
    NULL;
...
setup_signal (int sig)
{
  ...
  sigact.sa_sigaction =
      (void (*)(int signal, siginfo_t * info,
          void *ptr)) sharedwine_signal_handler;
...
static TEB *
shared_alloc_teb (ULONG * size)
{
  TEB *teb;

  *size = SIGNAL_STACK_SIZE + sizeof (TEB);
  teb =
      (TEB *) wine_anon_mmap (NULL, *size, PROT_READ | PROT_WRITE | PROT_EXEC,
      0);
  if (teb == (TEB *) - 1)
    return NULL;
...
new_thread_proxy (LPVOID param)
{
  ...
  while (1) {
    ...
    if (read (proxy_pripe_request[0], &sri, sizeof (sri)) != sizeof (sri)) {
      printf ("cannot read new thread request from proxy request pipe (%s)",
          strerror (errno));
      return (DWORD) - 1;
    }
    ...
    if (write (proxy_pripe_status[1], &sri, sizeof (sri)) != sizeof (sri)) {
      printf ("cannot write request/reply back to proxy reply pipe (%s)",
          strerror (errno));
      /* XXX free teb */
      return (DWORD) - 1;
    }
    ...
    if (write (proxy_pripe_status[1], &fd, sizeof (fd)) != sizeof (status)) {
      printf ("cannot write status back to proxy reply pipe (%s)",
          strerror (errno));
      /* XXX free teb */
      return (DWORD) - 1;
    }

    if (write (proxy_pripe_status[1], &thread_info,
            sizeof (thread_info)) != sizeof (thread_info)) {
      printf ("cannot write thread info to proxy reply pipe (%s)",
          strerror (errno));
      /* XXX free teb */
      return (DWORD) - 1;
    }
...
#define GET_SYMBOL(res,type,handle,name) if((res=(type)WineGetProcAddress (handle,name))==NULL) { printf("cannot find Windows function %s", name); return -1; }
  ...
  GET_SYMBOL (ntallocatevm, NtAllocateVirtualMemory_ptr, ntdll,
      "NtAllocateVirtualMemory");
  GET_SYMBOL (ntfreevm, NtFreeVirtualMemory_ptr, ntdll, "NtFreeVirtualMemory");
  GET_SYMBOL (ntprotectvm, NtProtectVirtualMemory_ptr, ntdll,
      "NtProtectVirtualMemory");
  GET_SYMBOL (acquirepeblock, RtlAcquirePebLock_ptr, ntdll,
      "RtlAcquirePebLock");
  GET_SYMBOL (releasepeblock, RtlReleasePebLock_ptr, ntdll,
      "RtlReleasePebLock");
...
wine_adopt_thread (void)
{
  ...
  SERVER_START_REQ (new_thread) {
    ...
    if (write (proxy_pripe_request[1], &__req, sizeof (__req)) != sizeof (__req)
        || read (proxy_pripe_status[0], &__req,
            sizeof (__req)) != sizeof (__req)
        || read (proxy_pripe_status[0], &request_fd,
            sizeof (request_fd)) != sizeof (request_fd)
        || read (proxy_pripe_status[0], &thread_info,
            sizeof (thread_info)) != sizeof (thread_info))
    {
  ...
  ntallocatevm (GetCurrentProcess (), &teb->DeallocationStack,
      thread_info.stack_base, &size, MEM_SYSTEM, PAGE_EXECUTE_READWRITE);
