 ...
 */
     void wine_pthread_init_process (const struct wine_pthread_functions
    *functions)
{
...
void
wine_pthread_get_stack_info (void **stack_base, size_t * stack_size)
{
...
void
wine_pthread_init_thread (struct wine_pthread_thread_info *info)
{
 ...
 */
void
wine_pthread_init_current_teb (struct wine_pthread_thread_info *info)
{
#ifdef __i386__
 ...
 */
int
wine_pthread_create_thread (struct wine_pthread_thread_info *info)
{
  pthread_t id;
  pthread_attr_t attr;
  int ret = 0;
 ...
 */
void *
wine_pthread_get_current_teb (void)
{
#ifdef __i386__
  void *ret;
__asm__ (".byte 0x64\n\tmovl 0x18,%0":"=r" (ret));
 ...
 */
void
wine_pthread_exit_thread (struct wine_pthread_thread_info *info)
{
  struct cleanup_info
  {
    pthread_t self;
  ...
  cleanup_info =
      (struct cleanup_info *) ((char *) info->teb_base + info->teb_size) - 1;
  cleanup_info->self = pthread_self ();
  cleanup_info->thread_info = *info;

  if ((free_info =
          interlocked_xchg_ptr ((void **) &previous_info,
              cleanup_info)) != NULL) {
 ...
 */
void
wine_pthread_abort_thread (int status)
{
