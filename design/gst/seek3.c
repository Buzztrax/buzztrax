    ...
    switch (message->type) {
      ...
      case GST_MESSAGE_SEGMENT_DONE: {
        event = gst_event_new_seek(1.0, GST_FORMAT_TIME,
          ...
          GST_SEEK_TYPE_SET, (GstClockTime)10 * GST_SECOND);
...
main (gint argc, gchar ** argv)
{
  ...
  for(i=0;i<5;i++) {
    ...
    gst_controller_set (ctrl1, "volume", i * 2 * GST_SECOND, &vol);
    g_value_set_double (&vol, 1.0);
    gst_controller_set (ctrl1, "volume", 1 + i * 2 * GST_SECOND, &vol);
  }

  // configure source 2
  g_object_set(src2,"freq",1000.0,NULL);
  // add a controller to the source1
  ...
  for(i=0;i<5;i++) {
    g_value_set_double (&vol, 1.0);
    gst_controller_set (ctrl2, "volume", i * 2 * GST_SECOND, &vol);
    g_value_set_double (&vol, 0.0);
    gst_controller_set (ctrl2, "volume", 1 + i * 2 * GST_SECOND, &vol);
  }
