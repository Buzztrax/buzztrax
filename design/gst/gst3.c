...
GstPad *
gst_element_get_first_pad_by_direction (GstElement * element,
...
main (int argc, char **argv)
{
  ...
  if (!(gst_element_link (audiosrc, audiosink))) {
    ...
    gst_element_add_ghost_pad (audioadaptor, gst_element_get_pad (conv1,
            "sink"), "sink");
    gst_element_add_ghost_pad (audioadaptor, gst_element_get_pad (conv2, "src"),
        "src");
  ...
  if (gst_element_set_state (thread, GST_STATE_PLAYING)) {
    ...
    while (vol_doubleval > 0.0) {
      g_print ("tick : vol=%f frq=%f - clock time %" G_GUINT64_FORMAT
          " - sink time %" G_GUINT64_FORMAT "\n", vol_doubleval, frq_doubleval,
          gst_clock_get_time (clock), gst_element_get_time (audiosink));
      //-- update params
      g_object_set_property (G_OBJECT (vol), "value_double", vol_val);
      g_object_set_property (G_OBJECT (frq), "value_double", frq_val);
      //-- update values
    ...
    g_print ("starting to play failed\n");
