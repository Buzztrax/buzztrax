...
static void
timing (const guint rate, const gdouble ticks_per_minute)
{
  guint i;
  ...
  printf ("wait per position=%lf, time per sample=%lf, sample per buffer=%lf\n",
      ...
      ((gdouble) GST_SECOND / (gdouble) rate), ideal_samples_per_buffer);
  ...
  for (i = 0; i < 15; i++) {
    ...
    printf ("%2u : %15" G_GUINT64_FORMAT " %15" G_GUINT64_FORMAT " %15"
        G_GINT64_FORMAT " : %15" G_GINT64_FORMAT " %15" G_GINT64_FORMAT " : %15"
        G_GINT64_FORMAT " %lf=%11.5lf-%11.5lf\n", i, ts, qtime, qdiff,
        (guint64) atime, adiff, aqdiff, adjust, (gdouble) asamples,
        samples_done);
...
main (int argc, char **argv)
{
  ...
  if (argc > 1)
    rate = atoi (argv[1]);
  if (argc > 2)
    beats_per_minute = atoi (argv[2]);
  if (argc > 3)
    ticks_per_beat = atoi (argv[3]);
