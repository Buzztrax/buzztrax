...
setup_network (gchar * master)
{
  ...
  if (!(bt_connection_new (thread, gen1, sink))) {
    GST_DEBUG ("can't connect machines: gen1,sink\n");
    exit (1);
  }
  if (!(bt_connection_new (thread, gen2, sink))) {
    GST_DEBUG ("can't connect machines: gen2,sink\n");
    exit (1);
  }
...
setup_sequence (void)
{
  ...
  tracks = (BTPatternPtr *) timeline[0];
  tracks[0] = NULL;
  tracks[1] = NULL;
  tracks = (BTPatternPtr *) timeline[1];
  tracks[0] = pat[0];
  tracks[1] = NULL;
  tracks = (BTPatternPtr *) timeline[2];
  tracks[0] = NULL;
  tracks[1] = pat[1];
  tracks = (BTPatternPtr *) timeline[3];
  tracks[0] = pat[0];
  tracks[1] = pat[1];
}

void
play_timeline (BTPatternPtr * tracks, guint position)
{
  guint track, ticks;
...
play_sequence (void)
{
  ...
  if (gst_element_set_state (thread, GST_STATE_PLAYING)) {
    ...
    GST_INFO ("starting to play failed\n");
}
