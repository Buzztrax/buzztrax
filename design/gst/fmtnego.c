 ...
 *
 * gcc -Wall -g fmtnego.c -o fmtnego `pkg-config gstreamer-0.10 --cflags --libs`
...
static void
on_format_negotiated (GstPad * pad, GParamSpec * arg, gpointer user_data)
{
  GstCaps *pad_caps;
  ...
  if ((pad_caps = gst_pad_get_negotiated_caps (pad))) {
    ...
    if (!target_caps || !gst_caps_get_size (target_caps)) {
      ...
      if ((ps = gst_caps_get_structure (pad_caps, 0))) {
        ...
        if (!strcmp (p_name, "audio/x-raw-int"))
          fmt_data->type = 0;
        else if (!strcmp (p_name, "audio/x-raw-float"))
          fmt_data->type = 1;
      ...
      GstStructure *ps;
      GST_INFO ("target caps %d, pad caps %d", gst_caps_get_size (target_caps),
          gst_caps_get_size (pad_caps));
      ...
      if ((ps = gst_caps_get_structure (pad_caps, 0))) {
        ...
        GST_INFO ("target channels %d, pad channels %d", fmt_data->channels,
            p_channels);
    ...
    if (new_caps) {
      ...
      if (channels)
        gst_structure_set (ns, "channels", GST_TYPE_INT_RANGE,
            fmt_data->channels, 8, NULL);
      if (width)
        gst_structure_set (ns, "width", GST_TYPE_INT_RANGE, fmt_data->width, 32,
            NULL);
      if (depth)
        gst_structure_set (ns, "depth", GST_TYPE_INT_RANGE, fmt_data->depth, 32,
            NULL);
...
main (int argc, char **argv)
{
  ...
  if ((conv1_sink_pad = gst_element_get_pad (conv1, "sink"))) {
    g_signal_connect (conv1_sink_pad, "notify::caps",
        G_CALLBACK (on_format_negotiated), (gpointer) & fmt_data);
  }
  gst_object_unref (conv1_sink_pad);
  if ((conv2_sink_pad = gst_element_get_pad (conv2, "sink"))) {
    g_signal_connect (conv2_sink_pad, "notify::caps",
        G_CALLBACK (on_format_negotiated), (gpointer) & fmt_data);
  }
  gst_object_unref (conv2_sink_pad);

  /* add objects to the main bin */
  gst_bin_add_many (GST_BIN (bin), src1, src2, pan, mix, conv1, conv2, conv3,
      fmtflt, sink, NULL);
