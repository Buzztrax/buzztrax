 ...
 *
 * gcc -Wall -g `pkg-config gtk+-2.0 libgnomecanvas-2.0 --cflags --libs` canvas-redraw.c -o canvas-redraw
 */
...
static void on_canvas_size_allocate(GtkWidget *widget,GtkAllocation *allocation,gpointer user_data) {
  ...
  sprintf(title,"canvas demo [canvas size %4d x %4d]",allocation->width,allocation->height);
