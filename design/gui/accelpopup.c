...
static void
on_menu_activate (GtkMenuItem * menuitem, gpointer user_data)
{
  gtk_label_set_text (GTK_LABEL (user_data), "main menu");
}

static void
on_context_menu1_activate (GtkMenuItem * menuitem, gpointer user_data)
{
  gtk_label_set_text (GTK_LABEL (user_data), "context menu 1");
}

static void
on_context_menu2_activate (GtkMenuItem * menuitem, gpointer user_data)
{
  gtk_label_set_text (GTK_LABEL (user_data), "context menu 2");
}

static void
on_popup_button_clicked (GtkButton * widget, gpointer user_data)
{
  gtk_menu_popup (GTK_MENU (user_data), NULL, NULL, NULL, NULL, 3,
      gtk_get_current_event_time ());
}

static void
on_check_toggled (GtkToggleButton * togglebutton, gpointer user_data)
{
...
init ()
{
  ...
  g_signal_connect (G_OBJECT (mi), "activate", G_CALLBACK (on_menu_activate),
      (gpointer) label);
  ...
  gtk_menu_item_set_accel_path (GTK_MENU_ITEM (mi),
      "<Main>/Context1/PopupItem");
  gtk_accel_map_add_entry ("<Main>/Context1/PopupItem", GDK_P,
      GDK_CONTROL_MASK);
  gtk_menu_shell_append (GTK_MENU_SHELL (pm), mi);
  gtk_widget_show (mi);
  g_signal_connect (G_OBJECT (mi), "activate",
      G_CALLBACK (on_context_menu1_activate), (gpointer) label);

  button = gtk_button_new_with_label ("Popup 1");
  g_signal_connect (G_OBJECT (button), "clicked",
      G_CALLBACK (on_popup_button_clicked), (gpointer) pm);
  ...
  mi = gtk_menu_item_new_with_label ("popup item");
  gtk_menu_item_set_accel_path (GTK_MENU_ITEM (mi),
      "<Main>/Context2/PopupItem");
  gtk_accel_map_add_entry ("<Main>/Context2/PopupItem", GDK_O,
      GDK_CONTROL_MASK);
  gtk_menu_shell_append (GTK_MENU_SHELL (pm), mi);
  gtk_widget_show (mi);
  g_signal_connect (G_OBJECT (mi), "activate",
      G_CALLBACK (on_context_menu2_activate), (gpointer) label);
  ...
  g_signal_connect (G_OBJECT (ck), "toggled", G_CALLBACK (on_check_toggled),
