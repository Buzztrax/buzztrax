...
// zoom controls
static void on_zoom_in(GtkToolButton *toolbutton, gpointer user_data) {
...
static void on_zoom_reset(GtkToolButton *toolbutton, gpointer user_data) {
  GnomeCanvas *canvas = GNOME_CANVAS(user_data);
...
static void on_zoom_out(GtkToolButton *toolbutton, gpointer user_data) {
  GnomeCanvas *canvas = GNOME_CANVAS(user_data);
...
int main(int argc, char **argv) {
  ...
  g_signal_connect(G_OBJECT(ti), "clicked",	G_CALLBACK (on_zoom_in), (gpointer)canvas);
  gtk_toolbar_insert (GTK_TOOLBAR(tb), ti, -1);
  ti = gtk_tool_button_new_from_stock (GTK_STOCK_ZOOM_100);
  g_signal_connect(G_OBJECT(ti), "clicked",	G_CALLBACK (on_zoom_reset), (gpointer)canvas);
  gtk_toolbar_insert (GTK_TOOLBAR(tb), ti, -1);
  ti = gtk_tool_button_new_from_stock (GTK_STOCK_ZOOM_OUT);
  g_signal_connect(G_OBJECT(ti), "clicked",	G_CALLBACK (on_zoom_out), (gpointer)canvas);
  gtk_toolbar_insert (GTK_TOOLBAR(tb), ti, -1);
