...
static void init() {
  ...
  // colorize rows
  range.col0=0;range.coli=9;
  for(i=0;i<64;i+=2) {
    range.row0=i;range.rowi=i;
    gtk_sheet_range_set_background(GTK_SHEET(sheet), &range, &source_bg1);
    range.row0=i+1;range.rowi=i+1;
    gtk_sheet_range_set_background(GTK_SHEET(sheet), &range, &source_bg2);
  }
  
  // global justification
  range.col0=0;range.coli=9;
  range.row0=0;range.rowi=63;
  gtk_sheet_range_set_justification(GTK_SHEET(sheet), &range, GTK_JUSTIFY_RIGHT);
  ...
  range.row0=0;range.rowi=63;
  gtk_sheet_range_set_border(GTK_SHEET(sheet), &range, GTK_SHEET_RIGHT_BORDER, 1, GDK_LINE_SOLID);
