...
GType
gst_trigger_switch_get_type (void)
{
  static GType type = 0;
  if (type == 0) {
...
static void
selection_cell_data_function (GtkTreeViewColumn * col,
    ...
    gpointer user_data)
{
  ...
  if ((enum_value = g_enum_get_value (enum_class, value))) {
    g_object_set (G_OBJECT (renderer), "text", enum_value->value_name, NULL);
...
init ()
{
  ...
  g_object_set (GTK_TREE_VIEW (tree_view), "enable-search", FALSE, "rules-hint",
      TRUE, NULL);
  ...
  gtk_tree_selection_set_mode (gtk_tree_view_get_selection (GTK_TREE_VIEW
          (tree_view)), GTK_SELECTION_BROWSE);

  renderer = gtk_cell_renderer_text_new ();
  g_object_set (G_OBJECT (renderer), "xalign", 1.0, NULL);
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (tree_view), -1,
      "Col 1", renderer, "text", 0, NULL);

  renderer = gtk_cell_renderer_text_new ();
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (tree_view), -1,
      "Col 2", renderer, "text", 1, NULL);

  renderer = gtk_cell_renderer_text_new ();
  if ((tree_col =
          gtk_tree_view_column_new_with_attributes ("Col 3", renderer, "text",
              2, NULL))) {
    gtk_tree_view_append_column (GTK_TREE_VIEW (tree_view), tree_col);
    gtk_tree_view_column_set_cell_data_func (tree_col, renderer,
        selection_cell_data_function, NULL, NULL);
  }
  ...
  // list store
  store =
      gtk_list_store_new (3, G_TYPE_INT, G_TYPE_INT, GST_TYPE_TRIGGER_SWITCH);

  for (i = 0; i < 15; i++) {
    gtk_list_store_append (store, &tree_iter);
    gtk_list_store_set (store, &tree_iter, 0, i, 1, i * i, 2, triggers[i % 3],
        -1);
  }
