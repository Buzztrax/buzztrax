...
static void
bt_song_real_start_play (BtSong * self)
{
  /* emitting signal if we start play */
  g_signal_emit (self, BT_SONG_GET_CLASS (self)->play_signal_id, 0, NULL);
}

/* play method from song */
void
bt_song_start_play (BtSong * self)
{
  BT_SONG_GET_CLASS (self)->start_play (self);
}

static void
song_get_property (GObject * object,
...
static void
song_set_property (GObject * object,
...
static void
bt_song_init (GTypeInstance * instance, gpointer g_class)
{
...
static void
bt_song_class_init (BtSongClass * klass)
{
  ...
  klass->play_signal_id = g_signal_new ("play", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST | G_SIGNAL_NO_RECURSE | G_SIGNAL_NO_HOOKS, (guint) G_STRUCT_OFFSET (BtSongClass, play_event), NULL,        // accumulator
      NULL,                     // acc data
