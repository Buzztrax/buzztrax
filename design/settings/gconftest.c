...
gboolean test(void) {
  ...
  if(client) {
    ...
    gconf_client_add_dir(client,BT_GCONF_PATH_GNOME,GCONF_CLIENT_PRELOAD_ONELEVEL,NULL);

    // read a value
    value=gconf_client_get_string(client,BT_GCONF_PATH_GNOME"/toolbar_style",NULL);
