...
static void
device_added (LibHalContext * ctx, const gchar * udi)
{
  gchar **cap;
  gchar *parent_udi;
  size_t n;
  ...
  if (!(cap =
          libhal_device_get_property_strlist (ctx, udi, "info.capabilities",
              NULL))) {
    return;
  }
  ...
  for (n = 0; cap[n]; n++) {
    if (!strcmp (cap[n], "alsa")) {
      parent_udi =
          libhal_device_get_property_string (ctx, udi, "info.parent", NULL);
      parent_udi =
          libhal_device_get_property_string (ctx, parent_udi, "info.parent",
          NULL);

      printf ("alsa device added: type=%s, device_file=%s, vendor=%s\n",
          libhal_device_get_property_string (ctx, udi, "alsa.type", NULL),
          libhal_device_get_property_string (ctx, udi, "alsa.device_file",
              NULL), libhal_device_get_property_string (ctx, parent_udi,
              "info.vendor", NULL)
          );
